...
export const resolveFrontHeights = (ctx: RuleContext): Mm[] => {
  ...
  const opening = mm(ctx.H - 2 * ctx.construction.frontReveal);
  return equalDrawerFronts(opening, count, ctx.construction.frontGap);
};
...
export const DRAWER_BANK_SPEC: CabinetSpec = {
  ...
  validate: (ctx) => {
    ...
    if (heights && heights.length > 0) {
      const gaps = ctx.construction.frontGap * (heights.length - 1);
      const used = heights.reduce((a, b) => a + b, 0) + gaps + 2 * ctx.construction.frontReveal;
      if (used > ctx.H + 0.5) {
        problems.push(
