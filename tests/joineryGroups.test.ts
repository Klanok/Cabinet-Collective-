...
describe('every setting has somewhere to be', () => {
  ...
  it('covers the whole method, so the count cannot drift quietly', () => {
    // Hand-worked: 32 keys on a construction method, less id, name and family, is 29 settings.
    // 27 before §4.27, then 30 when it added `cornerMethod` and two routed figures, then 29 when
    // the shop said the relief is one continuous pocket and the pitch stopped meaning anything.
    // This assertion caught both moves, which is the whole point of counting against the model.
    const grouped = JOINERY_GROUPS.flatMap((g) => g.keys).length;
    expect(settingKeys.length).toBe(29);
    expect(grouped).toBe(29);
  });
